...
int Jeu(RenderWindow *fenetre, int type)
{
	...
	background_text.loadFromFile("ressources/img/backgrounds/temple-1.png");
	background2_text.loadFromFile("ressources/img/backgrounds/temple-2.png");
	...
	Joueur joueur1(Vector2f(LARGEUR_TERRAIN/2 + 100, 168));
	Joueur joueur2(Vector2f(LARGEUR_TERRAIN/2 - 100, 468), DROITE);

	std::cout << "Je suis encore en vie :)\n";

	//On définit ici les touches de chaque joueur
	//L'ordre dans lequel on entre les touches est important ! (voir Joueur.h)
	joueur1.input.addKey(Keyboard::Left);
	joueur1.input.addKey(Keyboard::Up);
	joueur1.input.addKey(Keyboard::Right);
	joueur1.input.addKey(Keyboard::Down);
	joueur1.input.addKey(Keyboard::RControl);

	joueur2.input.addKey(Keyboard::Q);
	joueur2.input.addKey(Keyboard::Z);
	joueur2.input.addKey(Keyboard::D);
	joueur2.input.addKey(Keyboard::S);
	joueur2.input.addKey(Keyboard::LShift);

	joueur2.changerDirection(DROITE);

	//écran de sélection des personnages.
	selectionPersonnages(&joueur1, &joueur2);
	...
	*/
	...
	while(!quitter)
	{
		...
		joueur2.update(fps, &joueur1);
		joueur1.update(fps, &joueur2);
		...
		fenetre->clear(Color(164,202,213));
