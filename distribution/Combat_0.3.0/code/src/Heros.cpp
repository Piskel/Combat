...
Heros::Heros(Vector2f position, int direction)
{
	...
	fumee.loadTexture("ressources/img/effets/fumee.png");
	fumee.nbAnims = 10;
	fumee.dureeDeVie = seconds(0.5);
	feu.setNbParticules(20);
	feu.loadTexture("ressources/img/effets/feu.png");
	feu.nbAnims = 10;
...
/***********************************************
*
* Méthodes pour le déplaacement et les actions
*
************************************************/
...
{
	if ((!m_air) && (m_recover == seconds(0)) && (m_KO == seconds(0)))
	{
		m_roulade = true;
...
void Heros::update(float fps, Heros *ennemi)
{
	...
	if(m_PV == 0)
	{
		...
		if ((m_air) && (clockParticules.getElapsedTime() > seconds(1/FREQUENCE_PARTICULES)))
		{
	...
	else
	{
		...
		else if (m_roulade)
		{
			...
			if (ennemi->getPosition().x - getPosition().x > 0)
				m_direction = DROITE;
			else
				m_direction = GAUCHE;
		}
