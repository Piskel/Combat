...
#define FREQUENCE_PARTICULES 25.0 //nombre de fois par seconde qu'un particule peut apparître
...
class Heros
{
	...
	protected:
		void gravite(float fps); //calcule l'action de la gravite
		void demitour();

		Heros *m_ennemi; //un pointeur sur l'adversaire

		ParticleSystem fumee;
		ParticleSystem feu;
		...
		bool m_invincible;
		bool m_feu; //pour savoir si le héros est en feu (pour l'anim classe)
		int m_PV;
		float m_defense; //Indique à quel point le héros est résistant ou non aux coups (dégats + vol) : 1 = normal, 2 = 2*plus résistant.
		unsigned int m_coupID; //l'identifiant du coup, pour s'y retrouver. NORMAL, BAS, ... (NUL = aucun coup)
		unsigned int m_coupPrecedent;
		...
		sf::Time m_KO; //le temps où le héros sera KO, et ne peut rien faire (aussi utilisée pour les roulades et les retournements)
		...
		sf::Clock clockCoup;
		//Roulade :
		sf::Clock clockRoulade;
		sf::Clock clockParticules;
};
